...
def main() -> None:
    ...
    try:
        with connect_when_ready(port) as connection:
            ...
            connection.sendall(
                compressed_frame(0x0E, threshold, play_client_information)
            )
            connection.sendall(compressed_frame(0x06, threshold, encode_varint(0)))
            connection.sendall(
            ...
            connection.sendall(
                compressed_frame(
                    0x02,
                    threshold,
                    encode_varint(43) + encode_position(8, spawn_y - 1, 8),
                )
            )
            while True:
